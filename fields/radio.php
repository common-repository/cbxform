...
/**
 * Radio field defination of form element
 * Class CBXFormFieldRadio
 */
class CBXFormFieldRadio
{
    ...
    public static function previewhtml_drop($args = array()){
        ...
        return  '<div class="optionitems">
                                          <p class="optionitem optionitem-radio">
                                               <input class="optionval" type="radio" class="radio" name="cbxformmetabox[fields][formitem-#id#][formitem-#id#-1]" value="">
                                          ...
                                          <p class="optionitem optionitem-radio">
                                               <input class="optionval" type="radio" class="checkbox" name="cbxformmetabox[fields][formitem-#id#][formitem-#id#-2]" value="">
    ...
    public static function previewhtml_render( $form_item_id, $args = array() ){
        ...
        if (isset($args['option'])) {
            ...
            foreach ($args['option'] as $option_key => $option_val) {
                $radioinput_option_value = '<input type="text" class="optionlebel_value hidden" name="cbxformmetabox[fields][' . $form_item_id . '][option][' . $option_key . '][]" value="' . $option_val[0] . '"/>';
                $fieldhtml .= '<p class="optionitem optionitem-radio"><input type="radio" class="optionval" name="cbxformmetabox[fields][' . $form_item_id . '][' . $option_key . ']" value="' . $option_val[0] . '" > <span class="optionlebel">' . $option_val[0] . '</span>' . $radioinput_option_value;
            ...
            foreach ($args['option'] as $option_key => $option_val) {
                if (isset($_SESSION['cbxform_data']['error']) && sizeof($_SESSION['cbxform_data']['error']) > 0 && isset($_SESSION['cbxform_data']['data']) && sizeof($_SESSION['cbxform_data']['data']) > 0) {
                    if (array_key_exists($form_item_id, $_SESSION['cbxform_data']['data'])) {
                        if (in_array($option_key[0], $_SESSION['cbxform_data']['data'][$form_item_id])) {
                            $checked = 'checked';
                        } else {
                            $checked = '';
                        }
                ...
                $html .= '<div class="cbxradio-item">
                         <input type="radio" class="radio" name="cbxformmetabox[fields][' . $form_item_id . '][]" value="' . $option_key . '" id="' . $option_val[0] . '" ' . $required_string . ' ' . $checked . '/>
